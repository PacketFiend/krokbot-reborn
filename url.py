...
def title_auto(bot, trigger):
    ...
    #bot.say(str(results))
    #bot.say(str(short_urls))
    for title, short_url in zip(results, short_urls):
        message = '[ %s ] - %s' % (title[0], short_url)
...
def process_urls(bot, trigger, urls):
    ...
    for url in urls:
        if not url.startswith(bot.config.url.exclusion_char):
            ...
            #title = find_title(short_url)
            if title:
                results.append((title, get_hostname(url)))
                #results.append((title, get_hostname(short_url)))
    return results
